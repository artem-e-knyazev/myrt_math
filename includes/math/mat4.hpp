...
void TransposeAffine(mat4& dest, const mat4& src) {
    dest.px = src.px;
    dest.py = src.qx;
    dest.pz = src.rx;
    dest.pw = src.tx;
...
void InverseAffine(mat4& dest, const mat4& src) {
    TransposeAffine(dest, src);
    dest.t = -src.t;
}

mat4 InverseAffine(const mat4& m) {
    mat4 res;
    InverseAffine(res, m);
    return res;
}
